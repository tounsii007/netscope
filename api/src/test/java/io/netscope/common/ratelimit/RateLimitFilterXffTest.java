 ...
 *
 * After fix: clientIp() (now on {@link RateLimitIdentity}) ignores raw XFF
 * and uses ServletRequest.getRemoteAddr(), which Spring's
 * forward-headers-strategy=native + Tomcat's RemoteIpValve already populated
 * from XFF AFTER validating the immediate proxy is in the trusted-proxies
 * list.
 *
 ...
 *
 * The test targets {@link RateLimitIdentity#clientIp(jakarta.servlet.http.HttpServletRequest)}
 * directly — that's the identity-policy seam, and pinning it lets us refactor
 * {@link RateLimitFilter} (the HTTP-filter glue) without re-asserting the
 * spoofing-resistance invariant every time.
 */
class RateLimitFilterXffTest {
    ...
    @Test void clientIp_ignores_raw_XFF_header_attacker_spoofing() {
        ...
        assertThat(RateLimitIdentity.clientIp(req)).isEqualTo("198.51.100.42");
    }
    ...
    @Test void clientIp_uses_remoteAddr_for_random_XFF_attack() {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            assertThat(RateLimitIdentity.clientIp(req))
    ...
    @Test void clientIp_handles_null_remoteAddr_gracefully() {
        ...
        assertThat(RateLimitIdentity.clientIp(req)).isEqualTo("unknown");
    }
    ...
    @Test void clientIp_works_when_no_XFF_header_present() {
        ...
        assertThat(RateLimitIdentity.clientIp(req)).isEqualTo("8.8.8.8");
    }
